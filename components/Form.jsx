...
export default function Form() {
    ...
    return (
        <form data-aos='fade-in' data-aos-delay='250' onSubmit={submit} className='grid grid-cols-2 gap-4 w-full'
            acceptCharset='UTF-8'>

            <input required className='bg-gray-800 placeholder-white placeholder-opacity-50 rounded p-2 text-lg col-span-2 sm:col-span-1'
            ...
            <span className='relative inline-flex rounded-md justify-self-end'>
                ...
                {state.submitting ?
                    <button type='button'
                        ...
                        disabled=''>
                        <svg className='animate-spin -ml-1 mr-3 h-5 w-5 text-gray-300' xmlns='http://www.w3.org/2000/svg'
                ...
                <span className={state.submitted && !state.submitting ? 'hidden' : 'flex absolute h-3 w-3 top-0 right-0 -mt-1 -mr-1'}>
                    <span
                        className='animate-ping absolute inline-flex h-full w-full rounded-full bg-gradient-to-r from-myPrimary to-mySecondary opacity-75'></span>
